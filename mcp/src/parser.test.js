...
/**
 * test.js — Verify the parser and injector work correctly.
 * Creates a sample soul.md, parses it, and tests various injection scenarios.
 */

import { writeFileSync, unlinkSync } from "fs";
import { join } from "path";
import { parseSoul, parseSoulFromString, estimateTokens } from "./parser.js";
import { buildInjection } from "./injector.js";

const TEST_SOUL_PATH = join(process.cwd(), "_test_soul.md");
...
// Write sample soul
writeFileSync(TEST_SOUL_PATH, SAMPLE_SOUL);

try {
  console.log("\n  🧠 Manoma Core Test Suite\n");

  // ─── Parser tests ─────────────────────────────────────────────────

  console.log("  ─── Parser ───────────────────────\n");

  const soul = parseSoul(TEST_SOUL_PATH);
  const soulFromString = parseSoulFromString(SAMPLE_SOUL);

  assert("parseSoulFromString matches parseSoul", soul.config.default_mode === soulFromString.config.default_mode);
  assert("parses config", soul.config.default_mode === "work");
  assert("parses mode routing", Object.keys(soul.config.mode_routing).length >= 4);
  assert("parses identity section", !!soul.sections.identity);
  assert("parses values section", !!soul.sections.values);
  assert("parses voice section", !!soul.sections.voice);
  assert("parses skills section", !!soul.sections.skills);
  assert("parses skills.summary", !!soul.sections["skills.summary"]);
  assert("parses intuition section", !!soul.sections.intuition);
  assert("parses writing section", !!soul.sections.writing);
  assert("parses now section", !!soul.sections.now);
  assert("parses preferences section", !!soul.sections.preferences);
  assert("parses people section", !!soul.sections.people);
  assert("parses tools section", !!soul.sections.tools);
  assert("parses memory/decisions", !!soul.sections["memory/decisions"]);
  assert("parses memory/lessons", !!soul.sections["memory/lessons"]);
  assert("parses memory/positions", !!soul.sections["memory/positions"]);

  assert("extracts skill domains", soul.skillIndex.domains.length === 3);
  assert("extracts domain keywords", soul.skillIndex.domains[0].keywords.length > 0);
  assert("extracts person names", soul.personNames.length >= 3);
  assert("extracts person names correctly", soul.personNames.includes("jake"));
  assert("extracts person blocks", Object.keys(soul.personBlocks).length >= 3);

  assert("identity has correct meta", soul.sections.identity.meta.priority === "high");
  assert("identity inject = always", soul.sections.identity.meta.inject === "always");
  ...
  // Basic work mode
  const basic = buildInjection({ soul, mode: "work", userMessage: "hello" });
  assert("basic injection has content", basic.text.length > 0);
  assert("basic injection under budget", basic.totalTokens <= 2500);
  assert("includes IDENTITY", basic.sectionsUsed.includes("IDENTITY"));
  assert("includes VALUES", basic.sectionsUsed.includes("VALUES"));
  assert("includes VOICE", basic.sectionsUsed.includes("VOICE"));
  assert("includes SKILLS", basic.sectionsUsed.includes("SKILLS"));
  assert("includes INTUITION", basic.sectionsUsed.includes("INTUITION"));
  assert("includes owner name", basic.text.includes("Alex Chen"));

  // Decision trigger
  const decision = buildInjection({ soul, mode: "work", userMessage: "should I choose Postgres or Mongo?" });
  assert("decision trigger fires", decision.triggeredBy.includes("memory/decisions"));
  assert("decision trigger injects decisions", decision.sectionsUsed.some((s) => s.includes("DECISIONS")));

  // Skill domain trigger
  const apiQ = buildInjection({ soul, mode: "work", userMessage: "help me design the new API endpoint for users" });
  assert("skill keyword matches backend domain", apiQ.matchedDomains.length > 0);
  assert("skill domain depth injected", apiQ.sectionsUsed.some((s) => s.includes("SKILL DEPTH")));
  assert("skill behavior hint injected", apiQ.sectionsUsed.some((s) => s.includes("SKILL BEHAVIOR")));
  assert("skill behavior mentions level", apiQ.text.includes("expert") || apiQ.text.includes("Skip basics"));

  // Product strategy trigger
  const prodQ = buildInjection({ soul, mode: "work", userMessage: "how should we prioritize the roadmap?" });
  assert("product keywords trigger domain", prodQ.matchedDomains.some((d) => d.includes("product")));
  assert("product domain has skill behavior", prodQ.sectionsUsed.some((s) => s.includes("SKILL BEHAVIOR")));

  // Person trigger
  const personQ = buildInjection({ soul, mode: "work", userMessage: "write an email to Sarah about the API migration" });
  assert("person name triggers people", personQ.mentionedPeople.includes("sarah"));
  assert("person block injected", personQ.sectionsUsed.some((s) => s.includes("SARAH")));

  // Lesson trigger
  const lessonQ = buildInjection({ soul, mode: "work", userMessage: "what went wrong last time we launched?" });
  assert("lesson trigger fires", lessonQ.triggeredBy.includes("memory/lessons"));

  // Learning mode
  const learnQ = buildInjection({ soul, mode: "learning", userMessage: "teach me about Rust ownership" });
  assert("learning mode works", learnQ.mode === "learning");

  // Creative mode
  const creativeQ = buildInjection({ soul, mode: "creative", userMessage: "brainstorm a new landing page" });
  assert("creative mode works", creativeQ.mode === "creative");

  // Token budget
  const heavy = buildInjection({ soul, mode: "work", userMessage: "decide on API architecture and write an email to Sarah about the database strategy roadmap" });
  assert("heavy query stays under budget", heavy.totalTokens <= 2500);
  assert("heavy query has multiple triggers", heavy.triggeredBy.length >= 2);
